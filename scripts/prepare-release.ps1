...
$downloadName = $installerName.Replace(' ', '-')
$expectedUrl = 'https://github.com/Danny731/RHINE-ARCHIVE/releases/download/v' + $version + '/' + $downloadName
...
$sources = [ordered]@{}
$sources['RHINE-ARCHIVE.exe'] = Join-Path $projectRoot 'src-tauri/target/release/RHINE ARCHIVE.exe'
$sources[$downloadName] = Join-Path $bundleDir $installerName
$sources["$downloadName.sig"] = Join-Path $bundleDir "$installerName.sig"
...
$exe = Get-Item -LiteralPath $sources['RHINE-ARCHIVE.exe']
